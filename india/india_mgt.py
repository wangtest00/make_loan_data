...
def login_mgt():
    ...
    r=requests.post(india_mgt+'/api/login/auth?lang=en&lang=zh',data=json.dumps(data),headers=head_mgt,verify=False)
    check_api(r)
...
def approve(loan_no):
    ...
    r=requests.post(india_mgt+'/api/approve/distribution/case?lang=zh',data=json.dumps(data1),headers=head,verify=False)  #1.分配审批人员
    check_api(r)
    data2={"loanNo":loan_no,"decisionReason":"10280020","apprRemark":"test","approveResultType":"PASS"}
    r=requests.post(india_mgt+'/api/approve/handle/approve?lang=zh',data=json.dumps(data2),headers=head,verify=False)#2.审批通过
    check_api(r)
#批量分配审批人员及审批通过
...
def pl_shenpi():
    head=head_mgt_2()
    r=requests.get(india_mgt+'/api/approve/distribution/list?pageSize=10&pageNum=1&lang=zh',headers=head,verify=False)
    t=r.json()
    t=t['list']
    loan_No_List=[]
